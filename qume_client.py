...
class qume_api_methods(object):
    ...
    # returns the order book for a given contract market
    ...
    # signs a message
    def sign_message(self, endpoint, timestamp, method, post_url_string, post_body):
        message = bytes(str(post_body) + timestamp + method + endpoint + post_url_string, encoding='utf-8')
    ...
    def make_request(self, request_type, endpoint, post_url_string="", post_body=""):
        ...
        signature = self.sign_message(endpoint, timestamp, request_type, post_url_string, post_body)
